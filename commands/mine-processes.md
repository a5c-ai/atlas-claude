...
```
${CLAUDE_PLUGIN_ROOT}/processes/atlas-process-mining.mjs#process
```

(e.g. `babysitter run:create --process-id atlas-process-mining --entry "${CLAUDE_PLUGIN_ROOT}/processes/atlas-process-mining.mjs#process" --harness <this-harness>`, then iterate.) Pass the user arguments below as the run's stated need / process inputs. Continue executing in this same turn; do not stop after the Skill tool returns. Use the atlas skill and the Atlas MCP tools (mcp__atlas__atlas_public_*) for all graph queries.
