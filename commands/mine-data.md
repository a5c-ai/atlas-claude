...
(e.g. `babysitter run:create --process-id atlas-data-mining --entry "${CLAUDE_PLUGIN_ROOT}/processes/atlas-data-mining.mjs#process" --harness <this-harness>`, then iterate.) Pass the user arguments below as the run's stated need / process inputs. Continue executing in this same turn; do not stop after the Skill tool returns.
