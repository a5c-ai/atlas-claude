...
(e.g. `babysitter run:create --process-id atlas-collect-nuances --entry "${CLAUDE_PLUGIN_ROOT}/processes/atlas-collect-nuances.mjs#process" --harness <this-harness>`, then iterate.) Pass the user arguments below as the run's stated need / process inputs. Continue executing in this same turn; do not stop after the Skill tool returns.
